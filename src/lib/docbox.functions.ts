...
/** The shared Alice Lane Drive folder. Each company gets its own subfolder inside it,
 * created the first time a document is filed for that company. */
...
type Step = { key: number; title: string };
...
async function classifyStep(fileName: string, snippet: string, steps: Step[]) {
...
async function ensureCompanyFolder(companyName: string, cachedId: string | null) {
...
async function uploadToDrive(folderId: string, fileName: string, mimeType: string, bytes: ArrayBuffer) {
  const headers = driveHeaders();
  if (!headers) return null;
...
/** Classifies an already-uploaded DocBox file against the playbook's steps, pushes it to
 * the company's Google Drive folder, and records it against the meeting. */
