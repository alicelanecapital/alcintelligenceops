  ...
  return (
    <div className="max-w-7xl mx-auto px-8 py-10">
      <PageHeader
        ...
        actions={<Button className="bg-[var(--gold)] text-[var(--ink)] hover:bg-[var(--gold)]/90"><Sparkles className="h-4 w-4 mr-2" /> Run discovery</Button>}
      />
      {(q.data ?? []).length === 0 ? (
        <div className="rounded-lg border border-dashed border-border p-12 text-center bg-card">
          <Sparkles className="h-8 w-8 mx-auto text-[var(--gold)]" />
            ...
            <TableBody>
              {(q.data ?? []).map((e: any) => (
                <TableRow key={e.id}>
                  ...
                  <TableCell><Badge className="bg-[var(--gold)] text-[var(--ink)]">{e.ai_score ?? "—"}</Badge></TableCell>
