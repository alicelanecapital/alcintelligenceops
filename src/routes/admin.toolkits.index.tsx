      ...
      {!isDD && (
        <AlertDialog>
          <AlertDialogTrigger asChild>
            <Button size="icon" variant="ghost" className="h-7 w-7 text-destructive" title="Delete toolkit">
              <Trash2 className="h-3.5 w-3.5" />
            </Button>
          </AlertDialogTrigger>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Delete this toolkit?</AlertDialogTitle>
...
function NewToolkitButton() {
  ...
  const mut = useMutation({
    ...
    onError: (e: any) => toast.error(e.message ?? "Failed to create toolkit"),
  });
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button className="bg-primary text-primary-foreground hover:bg-primary/90">
          <Plus className="h-4 w-4 mr-2" /> New toolkit
        </Button>
      </DialogTrigger>
      <DialogContent>
