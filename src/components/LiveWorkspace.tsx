...
export type WorkspaceContext = "meeting" | "deal";

/** The Live Workspace. Rendered both for private meetings (/interviews/$id) and for
 * Deal Pipeline rooms (/dd-interview/...), where a DD playbook's rounds are the steps. */
export function LiveWorkspace({ id, context = "meeting", dealName, backTo, backLabel }: {
  id: string;
  ...
  return (
    <div className="min-h-screen">
      <div className="border-b border-border bg-white sticky top-0 z-10">
        <div className="max-w-[1600px] mx-auto px-8 py-4 flex items-center justify-between gap-4">
          <div className="flex items-center gap-4 min-w-0">
            <button onClick={() => nav({ to: (backTo ?? (isDeal ? "/dd-engine" : "/interviews")) as any })} className="text-muted-foreground hover:text-foreground" title={backLabel ?? (isDeal ? "Back to Deal Pipeline" : "Back to Meetings")}><ArrowLeft className="h-4 w-4" /></button>
            <div className="min-w-0">
              <div className="text-[10px] uppercase tracking-[0.2em] text-green-900">{isDeal ? "Deal Pipeline Room" : "Live Workspace"}</div>
      ...
      <LiveView interview={iv} reportAvailable={iv.status === "completed" || !!report.data} isDeal={isDeal} dealName={dealName ?? null} />
    </div>
  );
}
...
function LiveView({ interview, reportAvailable, isDeal, dealName }: { interview: any; reportAvailable: boolean; isDeal?: boolean; dealName?: string | null }) {
      ...
      <div className="mb-4">
        <div className="flex items-baseline justify-between mb-3 gap-2 flex-wrap">
          <div>
            ...
            <div className="text-[11px] text-muted-foreground -mt-1">Switch playbooks to see what other templates would examine in this same transcript — doesn't change what's already recorded.</div>
          </div>
      ...
      <WorkspaceGrid>
        ...
        <GridBlock panelKey="required_documents" layout={layout} className="min-w-0">
          <Card className="border-green-900/30"><CardContent className="p-0">
            ...
            {(stepDetail.data?.documents.length ?? 0) === 0 ? (
              <div className="p-4 bg-white text-xs text-muted-foreground italic">
                No required documents configured for this step.
              </div>
            ) : (
