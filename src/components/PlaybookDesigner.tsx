// Shared round/question/document designer, used by both /admin/dd-framework (the fixed
// Due Diligence toolkit) and /admin/toolkits/$id (any other toolkit, e.g. a custom "Pitch
// Playbook"). Rounds are scoped to a toolkit via dd_framework_rounds.toolkit_id -- the
// underlying round number is still a single counter shared across every toolkit.
...
export function PlaybookDesigner({ toolkitId, stripDueDiligenceWording }: { toolkitId: string; stripDueDiligenceWording?: boolean }) {
  const qc = useQueryClient();
  const rounds = useQuery({ queryKey: ["dd-framework-rounds", toolkitId], queryFn: () => fetchAllFrameworkRounds(toolkitId) });
  const [expanded, setExpanded] = useState<string[]>([]);

  const invalidate = () => {
    qc.invalidateQueries({ queryKey: ["dd-framework-rounds", toolkitId] });
  ...
  const addRoundMut = useMutation({
    mutationFn: () => createFrameworkRound(toolkitId),
    onSuccess: (created) => {
      qc.invalidateQueries({ queryKey: ["dd-framework-rounds", toolkitId] });
  ...
  const handleReorder = (newOrder: { round: number; title: string; subtitle?: string | null }[]) => {
    ...
    qc.setQueryData(["dd-framework-rounds", toolkitId], (prev: any) => {
    ...
    <div>
      ...
      {rounds.isSuccess && list.length === 0 ? (
        ...
        <SortableRoundAccordion
          rounds={list.map((r) => ({ round: r.round, title: strip(r.title), subtitle: strip(r.subtitle) }))}
          expanded={expanded}
